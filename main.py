...
# TEMPORARY FIX - Remove after updating all code
def load_all_whitelists():
    """Temporary function for compatibility"""
    return load_universal_whitelist()

def get_command_whitelist(command_name):
    """Temporary function for compatibility"""
    return get_universal_whitelist()

def get_command_whitelist_with_names(command_name):
    """Temporary function for compatibility"""
    return get_universal_whitelist_with_names()

def is_uid_allowed(command_name, uid):
    """Temporary function for compatibility"""
    return is_uid_allowed_in_universal(uid)




def get_player_name_enhanced(uid):
    """Enhanced function to get player name with multiple fallbacks"""
    try:
        # Method 1: Try your existing get_player_info function
        player_info = get_player_info(uid)
        if player_info and isinstance(player_info, dict) and "Name" in player_info:
            name = player_info["Name"]
            if name and name != "N/A":
                return name
        
        # Method 2: Try newinfo function if it exists
        try:
            info_result = newinfo(uid)
            if info_result and "status" in info_result and info_result["status"] == "ok":
                if "data" in info_result and "basicInfo" in info_result["data"]:
                    name = info_result["data"]["basicInfo"].get("nickname", "")
                    if name:
                        return name
        except:
            pass
        
        # Method 3: Return formatted UID
        return f"Player_{uid[:4]}"
        
    except Exception as e:
        # Return partial UID as fallback
        return f"UID_{uid[:6]}"
    ...
    try:
        with open(WHITELIST_FILE, "r") as f:
            for line in f:
                ...
                # Format: uid:custom_name
                if ":" in line:
                    parts = line.split(":", 1)
                    if len(parts) == 2 and parts[0].isdigit():
                        uid = parts[0]
                        custom_name = parts[1]
                        whitelist[uid] = custom_name
    except FileNotFoundError:
        # Create empty file
        with open(WHITELIST_FILE, "w") as f:
            f.write("# Universal Whitelist for ALL commands\n")
            f.write("# Format: uid:name (one per line)\n")
            f.write("# Example:\n")
            f.write("# 123456789:PlayerOne\n")
            f.write("# 987654321:PlayerTwo\n")
    return whitelist

def save_universal_whitelist(whitelist):
    """Save universal whitelist to file"""
    with open(WHITELIST_FILE, "w") as f:
        f.write("# Universal Whitelist for ALL commands\n")
        f.write("# Format: uid:name (one per line)\n\n")
        
        for uid, custom_name in whitelist.items():
            if uid.strip() and custom_name.strip():
                f.write(f"{uid}:{custom_name}\n")

def add_to_universal_whitelist(uid, custom_name):
    """Add a UID with custom name to universal whitelist"""
    whitelist = load_universal_whitelist()
    
    # Add or update UID with custom name
    whitelist[uid] = custom_name
    save_universal_whitelist(whitelist)
    return True, f"Added {uid} ({custom_name}) to universal whitelist"

def remove_from_universal_whitelist(target_uid):
    """Remove a UID from universal whitelist"""
    whitelist = load_universal_whitelist()
    
    if target_uid in whitelist:
        removed_name = whitelist[target_uid]
        del whitelist[target_uid]
        save_universal_whitelist(whitelist)
        return True, f"Removed {target_uid} ({removed_name}) from universal whitelist"
    return False, f"{target_uid} not found in universal whitelist"

def get_universal_whitelist():
    """Get all UIDs from universal whitelist (returns list)"""
    whitelist = load_universal_whitelist()
    return list(whitelist.keys())

def get_universal_whitelist_with_names():
    """Get all UIDs with names from universal whitelist (returns dict)"""
    return load_universal_whitelist()

def is_uid_allowed_in_universal(uid):
    """Check if UID is in universal whitelist"""
    whitelist = load_universal_whitelist()
    return str(uid) in whitelist

def is_master_admin(uid):
    """Check if UID is master admin"""
    return str(uid) in MASTER_ADMINS
# ===========================================
...
def load_admin_uids():
    """Load admin UIDs from file"""
    global ADMIN_UIDS
    try:
        with open("admin_uid.txt", "r") as f:
            ADMIN_UIDS = [line.strip() for line in f if line.strip()]
        print(f"✅ Loaded {len(ADMIN_UIDS)} admin UIDs")
    except FileNotFoundError:
        print("⚠️ admin_uid.txt not found. Creating empty file.")
        with open("admin_uid.txt", "w") as f:
            f.write("# Add admin UIDs here, one per line\n")
        ADMIN_UIDS = []
...
class MultiAccountManager:
    ...
    async def get_account_token(self, uid, password):
        ...
        try:
            ...
            return None, None
    # Add these functions for /lw command
...
async def auto_start_loop(team_code, uid, chat_id, chat_type, key, iv, region):
    ...
    # Set these values (you can change them if you want)
...
async def Room_Spam(Uid, Rm, Nm, K, V):
    ...
    fields = {
        ...
        2: {
            1: int(Rm),  
            2: "iG:[C][B][FF0000] COMPLEXITYCODEX",  
            3: {
                2: 1,
                3: 1
            },
                    ...
                    if response:
                        ...
                        if inPuTMsG.startswith(("/6")):
                            ...
                            try:
                                ...
                                msg = f"6-player group created for {player_name} [UID: ...{short_uid}]!"
                                P = await SEndMsG(current_chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                ...
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                                ...
                                                # Edit custom name for a UID
                                                # Edit custom name for a UID
                                                # Edit custom name for a UID
                                                # Edit custom name for a UID in universal whitelist
                        if inPuTMsG.strip().startswith('/whitelist_edit '):
                            # Check if user is master admin
                            if not is_master_admin(uid):
                                msg = "Permission denied. Only master admin can use this."
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                msg = "Usage: /whitelist_edit uid new_name\nExample: /whitelist_edit 123456789 NewName\nUse _ for spaces: New_Name"
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                continue
                            
                            target_uid = parts[1]
                            new_name = parts[2].replace("_", " ")
                            
                            # Check if UID exists in whitelist
                            allowed = is_uid_allowed_in_universal(target_uid)
                            if not allowed:
                                # Show only last 6 digits
                                short_uid = target_uid[-6:] if len(target_uid) > 6 else target_uid
                                msg = f"[UID: ...{short_uid}] not found in universal whitelist."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                continue
                            
                            # Get current whitelist
                            whitelist = get_universal_whitelist_with_names()
                            old_name = whitelist.get(target_uid, "Unknown")
                            
                            # Update the name
                            add_to_universal_whitelist(target_uid, new_name)
                            
                            # Show only last 6 digits
                            short_uid = target_uid[-6:] if len(target_uid) > 6 else target_uid
                            msg = f"Updated name for [UID: ...{short_uid}]:\nOld: {old_name}\nNew: {new_name}"
                            P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[B][C][FFC0CB]COMPLEXITY GUILD BOT

[C][B][FFFF00]ADMIN MESSAGE:

This bot is exclusively for members of the COMPLEXITY guild.

Features for our members:

- Auto-start bot for leveling
- Evolution emote spam
- Badge join requests
- Quick emote attacks
- And much more.
...
This bot is managed by our guild admin and is ONLY for Complexity members.


[C][B][0000FF] The bot is created by one of the Complexity's owners
"""
                            await safe_send_message(response.Data.chat_type, admin_message, uid, chat_id, key, iv)
                        # Tag command
                        if 'tag' in inPuTMsG.lower():
                            print(f"Help command detected from UID: {uid} in chat type: {response.Data.chat_type}")
    
                            # Header
                            header = """
                            [C][B][FFD700]══════════COMPLEXITY GUILD ══════════

                            [C][B][FFFFFF]Our Official Guild Tag:
                            """
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # Create spam packet
                                    spam_packet = await Room_Spam(target_uid, room_id, "Complexity", key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop_auto':
                            ...
                            if auto_start_running:
                                ...
                                success_msg = f"[B][C][00FF00]✅ SUCCESS! Auto start bot stopped!\nTeam: {auto_start_teamcode}\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                auto_start_teamcode = None
                            else:
                                error_msg = f"[B][C][FF0000]❌ ERROR! No auto start bot is running!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            if len(parts) < 2:
                                ...
                                # Start auto start
                                # REMOVED: global auto_start_task, stop_auto
                                stop_auto = False
...
"""
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            user_uid = str(uid)
                            user_is_whitelisted = is_uid_allowed_in_universal(user_uid)
                            #=========================================
    
                            # Header
                            header = """
[C][B][FFA500]═⚡ COMPLEXITY ⚡═[FFFF00][B]
├─ [FFFF00]Welcome to Complexity Guild Bot
└─ [FFFF00]See commands below
[FFFF00]━━━━━━━━━━━━[FFFF00]
"""
...
│  └─ [1E90FF]/bio [uid]
├─ [00CED1]Display tag of the guild
│  └─ [1E90FF]/tag
...
[FF69B4]━━━━━━━━━━━━[FF4500]"""
                            ...
                            await asyncio.sleep(0.2)
                                            ...
                                            # Whitelist Commands section


                            whitelist_help = """[C][B][808080]═⚡ WHITELIST COMMANDS ⚡═[FFFFFF]

                            ├─ [FFFFFF]Add UID 
│  └─ [C0C0C0]/whitelist_add [uid] [name]
├─ [FFFFFF]Remove UID
│  └─ [C0C0C0]/whitelist_remove [uid]
├─ [FFFFFF]View all whitelisted players
│  └─ [C0C0C0]/whitelist_view
├─ [FFFFFF]Check if UID is allowed
│  └─ [C0C0C0]/whitelist_check [uid]
├─ [FFFFFF]Edit player's name
│  └─ [C0C0C0]/whitelist_edit [uid] [new_name]
├─ [FFFFFF]Check your own access status
└─ [C0C0C0]/mywhitelist
"""
                            
                            await safe_send_message(response.Data.chat_type, whitelist_help, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
                            
                                                        # Footer
                            footer = """
[C][B][1E90FF]════⚡ INFO ⚡════[1E90FF][B]
├─ [FFFFFF]Guild: COMPLEXITY
├─ [FFFFFF]Status:[00FF00] ONLINE[1E90FF]
└─ [FFFFFF]Version: v1.0
[1E90FF]━━━━━━━━━━━━[1E90FF]
"""
                            ...
                            await safe_send_message(response.Data.chat_type, footer, uid, chat_id, key, iv)
                                                ...
                                                # ========== GENERAL WHITELIST COMMANDS ==========
                        
                        # Add UID to any command whitelist
                                                # ========== GENERAL WHITELIST COMMANDS ==========
                        
                                                # Add UID to any command whitelist WITH NAME
                                                # Add UID to any command whitelist WITH NAME
                                                # Add UID to any command whitelist WITH NAME
                                                # Add UID to universal whitelist
                        if inPuTMsG.strip().startswith('/whitelist_add '):
                            # Check if user is master admin
                            if not is_master_admin(uid):
                                msg = "Permission denied. Only master admin can use this."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                msg = "Usage: /whitelist_add uid name\nExample: /whitelist_add 123456789 PlayerName\nNote: If name has spaces, use _ instead: Player_Name"
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                continue
                            
                            new_uid = parts[1]
                            custom_name = parts[2].replace("_", " ")  # Convert _ back to spaces
                            
                            if not new_uid.isdigit():
                                msg = "Invalid UID. UID must be numbers only."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                continue
                            
                            success, result_msg = add_to_universal_whitelist(new_uid, custom_name)
                            
                            # Show only last 6 digits
                            short_uid = new_uid[-6:] if len(new_uid) > 6 else new_uid
                            msg = f"""The player is added seccessfully

                            [UID: ...{short_uid}] ({custom_name})"""
                            P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)


                        
                                                # Remove UID from command whitelist
                                                # Remove UID from command whitelist
                                                # Remove UID from command whitelist
                                                # Remove UID from universal whitelist
                        if inPuTMsG.strip().startswith('/whitelist_remove '):
                            # Check if user is master admin
                            if not is_master_admin(uid):
                                msg = "Permission denied. Only master admin can use this."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            ...
                            success, result_msg = remove_from_universal_whitelist(target_uid)
                            
                            # Show only last 6 digits
                            short_uid = target_uid[-6:] if len(target_uid) > 6 else target_uid
                            if success:
                                msg = f"Removed [UID: ...{short_uid}] from universal whitelist"
                            else:
                                msg = f"[UID: ...{short_uid}] not found in universal whitelist"
                        ...
                        if inPuTMsG.strip() == '/whitelist_commands':
                            # Check if user is master admin
                            if not is_master_admin(uid):
                                msg = "Permission denied. Only master admin can use this."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            ...
                            if not commands:
                                msg = "No commands have whitelists yet."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            else:
                                command_list = ", ".join(commands)
                                msg = f"Commands with whitelists: {command_list}\nUse /whitelist_view command to see players"
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                                ...
                                                # View UIDs for a specific comma
                                                # View UIDs with custom names for a specific command
                                                # View UIDs with custom names for a specific command
                                                # View UIDs with custom names for a specific command
                                                # View universal whitelist
                        if inPuTMsG.strip() == '/whitelist_view':
                            allowed_players = get_universal_whitelist_with_names()
                            
                            if not allowed_players:
                                msg = "No players in universal whitelist."
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            else:
                                # Create list with names and partial UIDs in brackets
                                player_list = []
                                for i, (player_uid, player_name) in enumerate(allowed_players.items(), 1):
                                    # Show only last 6 digits
                                    short_uid = player_uid[-6:] if len(player_uid) > 6 else player_uid
                                    player_list.append(f"{i}. {player_name} [UID: ...{short_uid}]")
                                
                                players_text = "\n".join(player_list)
                                msg = f"Universal Whitelist ({len(allowed_players)} players):\n{players_text}"
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                                ...
                                                # Check if a UID is allowed for a command
                                                # Check if a UID is allowed for a command
                                                # Check if a UID is in universal whitelist
                        if inPuTMsG.strip().startswith('/whitelist_check '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                msg = "Usage: /whitelist_check uid\nExample: /whitelist_check 123456789"
                                P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                continue
                            
                            check_uid = parts[1]
                            allowed = is_uid_allowed_in_universal(check_uid)
                            
                            # Get player name if available
                            players = get_universal_whitelist_with_names()
                            player_name = players.get(check_uid, "Unknown Player")
                            
                            # Show only last 6 digits
                            short_uid = check_uid[-6:] if len(check_uid) > 6 else check_uid
                            if allowed:
                                msg = f"[UID: ...{short_uid}] ({player_name}) CAN use ALL guild commands."
                            else:
                                msg = f"[UID: ...{short_uid}] ({player_name}) CANNOT use guild commands."
                            ...
                            await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                                ...
                                                # User can check their own status for all commands
                                                # User can check their own status for all commands
                                                # User can check their own status
                        if inPuTMsG.strip() == '/mywhitelist':
                            user_uid = str(uid)
                            allowed = is_uid_allowed_in_universal(user_uid)
                            
                            # Get player name if available
                            players = get_universal_whitelist_with_names()
                            player_name = players.get(user_uid, "You")
                            
                            # Show only last 6 digits
                            short_uid = user_uid[-6:] if len(user_uid) > 6 else user_uid
                            
                            if allowed:
                                msg = f"Your [UID: ...{short_uid}] ({player_name})\nStatus: ✅ ALLOWED for ALL guild commands"
                            else:
                                msg = f"Your [UID: ...{short_uid}] ({player_name})\nStatus: ❌ DENIED for guild commands\nNote: Commands work in private messages"
...
async def MaiiiinE():
    Uid , Pw = '4349719062','3539832311C4438DA781AB838EC4D083E5045584D04565631E1DF5059AC78779'
    ...
    os.system('clear')
        ...
        # Initialize universal whitelist if empty
    whitelist = load_universal_whitelist()
    if not whitelist:
        print("✅ Universal whitelist is empty. Add UIDs with /whitelist_add") 
    ...
    os.system('clear')
    print("Initializing COMPLEXITY Bot...")
    print("┌────────────────────────────────────┐")
    print("│ █████████████░░░░░░░░░░░░░░░░░░ │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
    ...
    os.system('clear')
    print(render('Complexitys', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 COMPLEXITYF BOT - ONLINE")
    print(f"🔹 UID: {TarGeT}")
    print(f"🔹 Name: {acc_name}")
    print(f"🔹 Status: 🟢 READY")
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using Complexity")
    sys.exit(0)
